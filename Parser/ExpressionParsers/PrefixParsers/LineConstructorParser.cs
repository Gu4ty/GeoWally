...
{
    class LineConstructorParser : IPrefixExpression
    {
        public LineConstructorParser(TopDownParser parser)
        {
            Register(parser);
        }
        public ExpressionNode Parse(TopDownParser parser, Token token)
        {
            ...
            {
                CompilingError expectedOpenBracket = new CompilingError(token.Location, ErrorCode.Expected, "Expected Open Bracket token after line keyword");
            ...
            {
                CompilingError argumentError = new CompilingError(token.Location, ErrorCode.Invalid, "Invalid constructor, line constructor expect two arguments");
                parser.AddError(argumentError);
                return null;
            }
            return new LineConstructorNode(Arguments[0], Arguments[1], token.Location);
        }

        public void Register(TopDownParser parser)
        {
            parser.RegisterPrefixParserByValue(TokenValues.Line, this);
        }
    }
}
