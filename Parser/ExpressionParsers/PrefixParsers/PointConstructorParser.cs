...
{
    class PointConstructorParser : IPrefixExpression
    {
        public PointConstructorParser(TopDownParser parser)
        {
            Register(parser);
        }
        public ExpressionNode Parse(TopDownParser parser, Token token)
        {
            ...
            return new PointConstructorNode(Arguments[0], Arguments[1], token.Location);
        }
