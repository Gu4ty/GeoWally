...
{
    class SegmentConstructorParser : IPrefixExpression
    {

        public SegmentConstructorParser(TopDownParser parser)
        {
            Register(parser);
        }
        public ExpressionNode Parse(TopDownParser parser, Token token)
        {
            ...
            {
                CompilingError expectedOpenBracket = new CompilingError(token.Location, ErrorCode.Expected, "Expected Open Bracket token after segment keyword");
            ...
            {
                CompilingError argumentError = new CompilingError(token.Location, ErrorCode.Invalid, "Invalid constructor, segment constructor expect two arguments");
                parser.AddError(argumentError);
                return null;
            }
            return new SegmentConstructorNode(Arguments[0], Arguments[1], token.Location);
        }

        public void Register(TopDownParser parser)
        {
            parser.RegisterPrefixParserByValue(TokenValues.Segment, this);
        }
    }
}
