...
{
    public class RayConstructorNode : ExpressionNode
    {
        ExpressionNode arg1, arg2;
        Type returnedType;
        bool isValidated;
        public RayConstructorNode(ExpressionNode p1, ExpressionNode p2, Compiling.CodeLocation location)
        {
            isValidated = false;
            returnedType = null;
            arg1 = p1;
        ...
        public override bool Validate(IContext context, InsideFunctions defaultFunctions, List<Compiling.CompilingError> errors)
        {
            ...
            if (arg1Type != typeof(GeoPoint) || arg2Type != typeof(GeoPoint))
            {
                ...
                {
                    argumentError = new Compiling.CompilingError(locationOfNode, Compiling.ErrorCode.Invalid, "First argument in ray constructor, argument must be a GeoPoint");
                    errors.Add(argumentError);
                }
                if (arg2Type != typeof(GeoPoint))
                {
                    argumentError = new Compiling.CompilingError(locationOfNode, Compiling.ErrorCode.Invalid, "Second argument in ray constructor, argument must be a GeoPoint");
                    errors.Add(argumentError);
                }
        ...
        public override GeoObject Run(IRunContext context, InsideFunctions defaultFunctions, IApplicationManager manager)
        {
            ...
            {
                if (param1 == null)
                    manager.ThrowException(locationOfNode, "Error in Ray constructor,first argument wrong");
                if (param2 == null)
                    manager.ThrowException(locationOfNode, "Error in Ray constructor,second argument wrong");
                return null;
            }
                return new GeoRay(param1, param2);
        }
    }
}
